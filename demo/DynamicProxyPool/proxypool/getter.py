...
class ProxyMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):  # 元类，动态添加类方法或属性
        count = 0
        attrs['__CrawlFunc__'] = []
        for k, v in attrs.items():
            if 'crawl_' in k:
                attrs['__CrawlFunc__'].append(k)  # 将类中指定方法添加到指定列表属性中
                count += 1
...
class FreeProxyGetter(object, metaclass=ProxyMetaclass):
    def get_raw_proxies(self, callback):
        ...
        for proxy in eval("self.{}()".format(callback)):  # 执行指定回调函数，获取返回值，回调函数为获取并返回原始代理地址
